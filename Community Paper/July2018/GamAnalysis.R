################################################################################
##  GamAnalysis.R This code analyzes the spreadsheet output by andrew do see when communities changed.
##
...
sig_only<-gam%>%
  ...
  filter(response_var != "composition_change"&response_var!="richness_change_abs")
...
gamtrts_metrics<-sig_only%>%
  left_join(trts_interactions)%>%
  ungroup()%>%
  filter(response_var != "composition_change"&response_var!="richness_change_abs", use == 1)%>%
