### Calculate synchrony and stability metrics
###
### Author: Kevin Wilcox (kevin.wilcox@uwyo.edu)
### Created: May 18, 2020, 2020; last updated: May 18, 2020
...
### NEED TO CHECK WHETHER THERE ARE KONZA PLOTS I NEED TO REMOVE
CDR_e001<-cover_df%>%
  filter(site_code=="CDR"&treatment==1|treatment==6|treatment==8|treatment==9)

CDR_e002<-cover_df%>%
  filter(site_code=="CDR"&treatment=='1_f_u_n'|treatment=='6_f_u_n'|treatment=='8_f_u_n'|treatment=='9_f_u_n')

full_df <- cover_df %>%
  filter(project_name!="e001"&project_name!="e002") %>%
  bind_rows(CDR_e001, CDR_e002) %>%
  left_join(exp_info, by=c("site_code", 
                           "project_name", 
                           "calendar_year",
                           "treatment_year",
                           "treatment",
...
###                                                                     ###
### Shaopeng's variance partitioning method
###                                                                     ###
