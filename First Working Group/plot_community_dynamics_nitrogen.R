...
gam_sub <- subset(gam_df, site_proj_comm=="NWTbowmanDryBowman" & metric%in%c("Evenness","Rank Corr."))
ggplot(data = gam_sub, aes(x=treatment_year))+
  geom_line(aes(y=yhat, linetype=metric))
...
####
serc_data <- subset(comm_metrics, site_proj_comm=="SERCCXN0"  & metric!="Precip.")
